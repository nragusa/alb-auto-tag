...
Description:
  Checks the scheme value of an Application Load Balancer, which can be one of internet-facing or
  internal. It will then check the tags on the ALB, and if there is a matching Key / Value pair
  that is required, it will return COMPLIANT. Otherwise, it will return NON_COMPLIANT. 

  Finally, if ENFORCE_COMPLIANCE is set to true, this rule will automatically update the 
  tag on the resource to become COMPLIANT. This should ultimately be moved to an SSM
  Document and the Config Rule should specify that as the remediation process.
...
Scenarios:
  ...
  Scenario: 2
     Given: There is at least 1 Application Load Balancer
       And: The ALB is internet-facing
       And: The ALB has a Tag Key=ExternalALB and Value=true
      Then: Return COMPLIANT
  Scenario: 3
     Given: There is at least 1 Application Load Balancer
       And: The ALB is internal
       And: The ALB has a Tag Key=InternalALB and Value=true
      Then: Return COMPLIANT
  Scenario: 4
     Given: There is at least 1 Application Load Balancer
       And: The ALB is internet-facing
       And: The ALB does not have a Tag Key=ExternalALB and Value=true
       And: ENFORCE_COMPLIANCE is set to false
      Then: Return NON_COMPLIANT
  Scenario: 5
     Given: There is at least 1 Application Load Balancer
       And: The ALB is internal
       And: The ALB does not have a Tag Key=InternalALB and Value=true
       And: ENFORCE_COMPLIANCE is set to false
      Then: Return NON_COMPLIANT
'''
import json
...
# External / Internal ALB tag key and value pairs
EXTERNAL_ALB_TAG_KEY = os.environ.get('EXTERNAL_TAG_KEY')
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for elb in all_elbv2:
        ...
        if overall_eval == 'NON_COMPLIANT':
            print('No compliant tag key / value pair found - marking as NON_COMPLIANT')
        evaluations.append(build_evaluation(
            elb['LoadBalancerArn'], overall_eval, event, annotation=get_str(overall_eval)))
    return evaluations


def get_str(compliance_value):
    if compliance_value == 'NON_COMPLIANT':
        return f"ALB must have a tag key of either {EXTERNAL_ALB_TAG_KEY} or {INTERNAL_ALB_TAG_KEY} and a value of 'true'"
    return None
...
def get_all_elbv2(client):
    ...
    while resp:
        ...
        resp = client.describe_load_balancers(
            Marker=resp['NextMarker']) if 'NextMarker' in resp else None
    return items


def get_alb_tags(client, elbv2_arn):
